...
void calculateProduct(struct Sparse_matrix a[100], struct Sparse_matrix b[100], int n1, int n2)
{
    ...
    int frow, fcol, sum = 0, k = 1, i, j, ii, jj;
    ...
    for (i = 1; i < n1;)
    {
        ...
        for (j = 1; j < n2;)
        {
            ...
            sum = 0;
            for (ii = 1, jj = 1; ii < n1 && jj < n2 && a[ii].row <= frow && b[jj].row <= fcol;)
            {
                if (a[ii].row != frow)
                    ii++;
                if (b[jj].row != fcol)
            ...
            if (sum != 0)
            {
                sparseProduct[k].row = frow;
                sparseProduct[k].col = fcol;
                sparseProduct[k].value = sum;
                sum = 0;
                k++;
            }

            while (j <= n2 - 1 && b[j].row == fcol)
                j++;
        }

        while (i <= n1 - 1 && a[i].row == frow)
            i++;
    }
    PRODUCT_SIZE = k;
